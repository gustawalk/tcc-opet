...
mod tests {
    ...
    #[test]
    fn returning_customer_lookup_uses_the_customer_created_date_index() {
        let conn = setup_db();
        let mut statement = conn
            .prepare(
                "EXPLAIN QUERY PLAN SELECT EXISTS(
                    SELECT 1 FROM service_orders previous
                    WHERE previous.customer_id = ?1
                      AND previous.deleted_at IS NULL
                      AND previous.created_date < date(?2)
                )",
            )
            .unwrap();
        let plan = statement
            .query_map(params!["customer-1", "2024-02-01"], |row| {
                row.get::<_, String>(3)
            })
            .unwrap()
            .collect::<Result<Vec<_>, _>>()
        ...
        assert!(plan
            .iter()
            .any(|detail| detail.contains("idx_service_orders_customer_created")));
    }
