  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      if (dialogRef.current) {
        setAnchoredTop(window.innerHeight / 2 - dialogRef.current.offsetHeight / 2);
      }
    });
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent
          ref={dialogRef}
          className="top-0 max-w-xl translate-y-0"
          style={anchoredTop === null ? undefined : { top: anchoredTop }}
        >
          <DialogHeader>
          ...
          <Input
            ...
            placeholder="Buscar clientes, OS, estoque ou modelos..."
          />

          <div className="max-h-80 space-y-4 overflow-y-auto">
            {term.trim().length < 2 && history.length > 0 && (
              <section>
                ...
                {history.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className="mt-1 flex w-full items-center gap-2 rounded-md px-2 py-2 text-left outline-none hover:bg-muted focus:bg-muted focus:ring-1 focus:ring-inset focus:ring-ring"
                      onClick={() => choose(item.path, item, item.kind)}
                    >
              ...
              <section>
                ...
                {quickActions.map((action) => (
                  <button
                    data-global-search-option
                    key={action.label}
                    className="mt-1 w-full rounded-md px-2 py-2 text-left text-sm outline-none hover:bg-muted focus:bg-muted focus:ring-1 focus:ring-inset focus:ring-ring"
                    onClick={action.run}
                  >
                    {action.label}
                  </button>
            ...
            {term.trim().length < 2 ? null : loading ? (
              <p className="text-sm text-muted-foreground">Buscando...</p>
            ) : groups.length === 0 && quickActions.length === 0 ? (
                  ...
                  <section key={group.label}>
                    ...
                    {group.items.map((item) => (
                      <button
                        ...
                        type="button"
                        className="mt-1 flex w-full items-center gap-2 rounded-md px-2 py-2 text-left outline-none hover:bg-muted focus:bg-muted focus:ring-1 focus:ring-inset focus:ring-ring"
                        onClick={() => choose(group.path, item, group.kind)}
                      >
                        <Icon className="h-4 w-4 shrink-0 text-muted-foreground" />
